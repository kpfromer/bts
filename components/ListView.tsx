                ...
                {event && (
                    <div className="p-8 rounded shadow-lg">
                        <div className="pb-3 mb-3 border-b-2 border-gray-300 space-y-3">
                            <h1 className="font-bold text-6xl text-logo font-header">{event.headliner}</h1>
                            <h2 className="font-bold text-3xl">{getEventDate(event).toLocaleString(DateTime.DATETIME_MED)}</h2>

                        </div>
                        <div dangerouslySetInnerHTML={{ __html: event.headlinerBio }} />

                        <div className="flex mt-3">
                            <button className="px-3 py-2 font-bold bg-logo rounded text-white ml-auto">Book</button>
                        </div>
                    </div>
                )}
            </div>
